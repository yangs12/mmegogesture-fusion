...
def load_models(fusion_type, quantized,device,num_classes,quant_type='qnnpack'):
  ...
  if 'concat' in fusion_type:
    ...
    model2.to(device)
    fusion_classifier = FusionClassifier(1280 * 2,num_classes).to(device)
...
@hydra.main(version_base=None, config_path="conf/avgresult_config/quant/camonly", config_name="camonly_inference_678")
