...
def main(args: DictConfig) -> None:
    ...
    for radar_file_path in sorted(radar_files):
        ...
        gesture_segments, center_indices, center_segments = gesture_detection(uD, args, uD_fps, radar_file_name)
    ...
    df.to_csv(os.path.join(args.process.output_dir, 'des_rpi_shubo_May26_12s_onefile.csv'), index=False)
    print(f"Radar mean: {np.mean(radar_mean)}, Radar std: {np.std(radar_mean)}")
