...
# Step 2: Load pretrained float32 weights BEFORE prepare/convert
...
# torch.ao.quantization.prepare(model_fp32, inplace=True)
# model_int8 = torch.ao.quantization.convert(model_fp32, inplace=True)
# model_int8.load_state_dict(torch.load(path_model, map_location=device, weights_only=False))
# # Load the quantized model directly
# # model_int8 = torch.load(path_model, map_location=device, weights_only=False)
