...
# labels = [0]*len(unique_episodes)  # Assuming all episodes belong to class 0 for this example
episode_i = 0
correct_samples = 0
for episode in sorted(unique_episodes):
    ...
    data_img = np.load(os.path.join(args.data_dir, f'{episode}-img.npy'))
    data_img = np.transpose(data_img, (2, 0, 1))  # Reorder dimensions from (H, W, C) to (C, H, W)
    data_uD = np.load(os.path.join(args.data_dir,f'{episode}-rad-uD.npy'))
    ...
    with torch.no_grad():
        ...
        if y_batch_pred.item() == label: #labels[episode_i]
            correct_samples += 1
        else:
            print(f'--------Episode {episode} Predicted class: {y_batch_pred.item()}, correct class: {label}') #labels[episode_i]
        episode_i += 1
    end = time.time()
