...
def main(args: DictConfig) -> None:
    ...
    for radar_file_path in radar_files:
        ...
        for i in range(len(gesture_segments)):
            start_idx, end_idx = center_segments[i]
            uD_segment = uD[:, start_idx:end_idx]
            # np.save(os.path.join(args.output_dir, f'{radar_file_name}-0-rad-uD.npy'), uD_segment)
            # save_uD_plot(uD_segment, args, radar_file_name, uD_axis)

            if args.if_resize_uD:
                # Resize the uD to the desired size
                uD_resized = cv2.resize(uD_segment, (args.uD_width, args.uD_height), interpolation=cv2.INTER_LINEAR)
                ...
                if args.if_save_uD:
                    np.save(os.path.join(args.output_resize_dir, f'{radar_file_name}-seg-{i}-Class{args.activity}-rad-uD.npy'), uD_resized)
                    save_uD_plot(uD_resized, args, radar_file_name + '-seg-' + str(i) + f'-Class{args.activity}_resized', uD_axis)

            if args.if_resize_img:
                video_path = os.path.join(args.folder_path, 'camera', radar_file_name + ".avi")
                frames = []
                ...
                middle_frame_index = int(center_indices[i]/  uD.shape[1] * args.capture_time * args.cam_fps)
                ...
                if args.if_save_img:
                    np.save(os.path.join(args.output_resize_dir, f'{radar_file_name}-seg-{i}-Class{args.activity}-img.npy'), img_resized)

                    cv2.imwrite(os.path.join(args.output_dir, f'{radar_file_name}-seg-{i}-Class{args.activity}-img.png'), img_resized)

        end_time = time.time()
        print(f"Time taken per seg: {(end_time - start_time)/len(gesture_segments)} seconds")
